...
while(cap.isOpened()):
    ...
    cv.putText(frame, str_up ,(10,40),font,0.6,(255,255,255),2,cv.LINE_AA)
    cv.putText(frame, str_up ,(10,40),font,0.6,(0,0,255),1,cv.LINE_AA)
    cv.putText(frame, str_down ,(10,90),font,0.6,(255,255,255),2,cv.LINE_AA)
    cv.putText(frame, str_down ,(10,90),font,0.6,(255,0,0),1,cv.LINE_AA)
    cv.putText(frame,f"Total number : {total}" ,(10,140),font,0.6,(255,0,0),1,cv.LINE_AA)
    cv.putText(frame, f"BAT {percent}% {plugged}" ,(10,190),font,0.6,(255,0,0),1,cv.LINE_AA)
    hsv = cv.cvtColor(frame,cv.COLOR_BGR2HSV)
    color = cv.applyColorMap(imggray, color_map)
    #hsv = cv.cvtColor(color,cv.COLOR_BGR2HSV)
    # display the color image
    cv.imshow('Thermal Image', color)
